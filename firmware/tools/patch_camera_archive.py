...
# Patching is deterministic given the same source archive, so skip it once
# already done — re-running ar/objcopy on every incremental build (most of
# which don't touch the camera) is pure overhead.
if not os.path.exists(archive) or os.path.getmtime(archive) < os.path.getmtime(source_archive):
